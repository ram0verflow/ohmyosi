...
// Known-answer tests from RFC 9001 Appendix A.2 (and RFC 9369 for v2): the
// complete protected client Initial packet, whose ClientHello names
// example.com. This is the test that matters - QUIC key derivation is the kind
// of code that compiles, runs, returns nothing, and looks exactly like a quiet
// network. A round trip against my own encryption would pass with the salt
// wrong; only a vector produced by someone else can catch that.

const rfc9001ClientInitialV1 = "" +
...
func TestQUICSNIRFCVectorV1(t *testing.T) {
	pkt := mustHex(t, rfc9001ClientInitialV1)
	if len(pkt) != 1200 {
		t.Fatalf("vector is %d bytes, want the full 1200-byte padded Initial", len(pkt))
	}
	if got := QUICSNI(pkt); got != "example.com" {
		t.Fatalf("QUICSNI = %q, want %q", got, "example.com")
	}
}

func TestQUICSNIRFCVectorV2(t *testing.T) {
	pkt := mustHex(t, rfc9369ClientInitialV2)
	if got := QUICSNI(pkt); got != "example.com" {
		t.Fatalf("v2 QUICSNI = %q, want %q", got, "example.com")
	}
}

// Anything that is not a readable client Initial must return "" rather than
// guessing: a wrong hostname is worse than no hostname.
func TestQUICSNIRejectsNonInitials(t *testing.T) {
	pkt := mustHex(t, rfc9001ClientInitialV1)
	for _, tc := range []struct {
		...
		{"empty", nil},
		{"short header packet", []byte{0x40, 1, 2, 3, 4, 5, 6, 7, 8}},
		{"handshake not initial", append([]byte{0xe0}, pkt[1:]...)},
		{"unknown version", append(append([]byte{}, pkt[0]), append([]byte{0xff, 0x00, 0x00, 0x1d}, pkt[5:]...)...)},
		{"plain udp payload", []byte("hello world, not quic at all")},
	} {
		if got := QUICSNI(tc.in); got != "" {
			t.Errorf("%s: got %q, want empty", tc.name, got)
		}
	}
}

// Truncation at every offset must return empty, never panic. Real captures are
// cut off by snaplen constantly.
func TestQUICSNITruncation(t *testing.T) {
	pkt := mustHex(t, rfc9001ClientInitialV1)
	for i := 0; i < len(pkt); i++ {
		func() {
			defer func() {
...
// A single flipped byte in the ciphertext must fail authentication and yield
// nothing, rather than producing garbage that looks like a hostname.
func TestQUICSNIRejectsTampered(t *testing.T) {
	pkt := mustHex(t, rfc9001ClientInitialV1)
	pkt[600] ^= 0x01
	if got := QUICSNI(pkt); got != "" {
		t.Errorf("tampered packet returned %q", got)
	}
}
